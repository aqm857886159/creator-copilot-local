...
function safeErrorMessage(error) {
  return String(error instanceof Error ? error.message : error ?? "未知错误")
    .replace(/(?:sk-[A-Za-z0-9_-]{16,}|[A-Za-z0-9+/=_-]{40,})/g, "[redacted]")
    .slice(0, 240);
}
...
parentPort.on("message", async (event) => {
  ...
  try {
    ...
    const facts = runtime.analysis.shotFacts({ workspaceId: payload.workspaceId, artifactId: payload.artifactId, shots, providerKey: "ffmpeg-scene", modelKey: "showinfo", contentHash: payload.contentHash, analysisRunId: payload.analysisRunId, createdAt });
    let asrStatus = "未配置本地 ASR 模型";
    let asrReady = false;
    try {
      if (typeof payload.whisperModelPath === "string" && payload.whisperModelPath) {
        const audio = await runtime.analysis.prepareWhisperAudio(payload.sourcePath);
        let segments;
        try {
          segments = await new runtime.analysis.WhisperCppTranscriber({ modelPath: payload.whisperModelPath, binaryPath: payload.whisperBinaryPath, language: payload.language ?? "zh" }).transcribe(audio.path);
        } finally {
          await audio.cleanup();
        }
        facts.push(...runtime.analysis.transcriptFacts({ workspaceId: payload.workspaceId, artifactId: payload.artifactId, segments, providerKey: "whisper.cpp", modelKey: path.basename(payload.whisperModelPath), contentHash: payload.contentHash, analysisRunId: payload.analysisRunId, createdAt }));
        ...
        const segments = await new runtime.analysis.FasterWhisperSidecarTranscriber({ modelPath: payload.fasterWhisperModelPath, scriptPath: payload.fasterWhisperScriptPath, pythonPath: payload.fasterWhisperPythonPath, language: payload.language ?? "zh", device: payload.fasterWhisperDevice, computeType: payload.fasterWhisperComputeType }).transcribe(payload.sourcePath);
        facts.push(...runtime.analysis.transcriptFacts({ workspaceId: payload.workspaceId, artifactId: payload.artifactId, segments, providerKey: "faster-whisper", modelKey: path.basename(payload.fasterWhisperModelPath), contentHash: payload.contentHash, analysisRunId: payload.analysisRunId, createdAt }));
        asrStatus = `ASR 已完成（${segments.length} 段）`;
        asrReady = true;
      }
    } catch (error) {
      asrStatus = `ASR 失败：${safeErrorMessage(error)}`;
    }
    ...
    if (process.platform === "darwin" && typeof payload.visionScriptPath === "string" && payload.visionScriptPath) {
      try {
        ...
        facts.push(...runtime.analysis.ocrFacts({ workspaceId: payload.workspaceId, artifactId: payload.artifactId, cues: mergedCues, providerKey: "apple-vision", modelKey: "VNRecognizeTextRequest", contentHash: payload.contentHash, analysisRunId: payload.analysisRunId, createdAt }));
        ocrStatus = `OCR 已完成（${mergedCues.length} 条，已合并重复花字）`;
        ocrReady = true;
      } catch (error) {
        ocrStatus = `OCR 失败：${safeErrorMessage(error)}`;
      }
    }
    ...
    parentPort.postMessage({ requestId, ok: false, error: safeErrorMessage(error) || "媒体分析 worker 失败" });
  }
});
