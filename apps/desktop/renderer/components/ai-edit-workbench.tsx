...
function AssetCandidateSection({ candidates, openFile }: { candidates: NonNullable<EditProposalResult["assetCandidates"]>; openFile: (relativePath: string) => Promise<void> }) {
  const sets = candidates.filter((set) => set.candidates.length > 0);
  if (sets.length === 0) return null;
  return <section className="edit-candidates"><div className="edit-section-heading"><div><div className="eyebrow">LOCAL CANDIDATES · REVIEW FIRST</div><h3>从素材库召回的候选画面</h3></div><span>{sets.reduce((count, set) => count + set.candidates.length, 0)} 个候选</span></div><p className="edit-candidate-note">候选只来自已经写入本地的 ASR、OCR 或镜头事实；它们不会自动替换 Take，先打开预览，再回到拍摄任务选择。</p>{sets.map((set) => <div className="edit-candidate-shot" key={set.shotId}><strong>分镜 {set.shotId}</strong><div className="edit-candidate-list">{set.candidates.map((candidate) => <article className="edit-candidate-row" key={`${set.shotId}-${candidate.assetId}`}><div><div className="edit-candidate-topline"><span>{candidate.confidence === "high" ? "高" : candidate.confidence === "medium" ? "中" : "低"}置信度 · {Math.round(candidate.score * 100)}%</span><span>{candidate.relativePath}</span></div><p>{candidate.reason}</p><small>证据：{candidate.evidenceIds.length > 0 ? candidate.evidenceIds.join("、") : "无文本命中，仅类型匹配"}{candidate.sourceSegment ? ` · ${seconds(candidate.sourceSegment.endMs - candidate.sourceSegment.startMs)} 片段` : ""}</small></div><button className="text-button" onClick={() => void openFile(candidate.relativePath)}><Search size={13} />打开预览</button></article>)}</div></div>)}</section>;
}
