...
日期：2026-08-14  
状态：合同、mock、一次真实 structured proposal smoke、proposal UI 调度持久化、freeze CommandReceipt 和 `submission_unknown` 人工收口 UI 已完成；Mastra durable resume 和跨进程恢复待继续
...
2. 将 render 失败重试和跨进程恢复写入已有 CommandReceipt、Job 和 ArtifactManifest；proposal/freeze/Provider 未知提交人工收口已接入；
