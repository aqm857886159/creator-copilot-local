...
- 已能从本地素材库生成可审阅 `EditProposal`，并通过 `AgentRuntimePort` 选择本地 fallback 或 APIMart AI SDK；提案请求和用户确认冻结已持久化 pending/accepted/rejected/submission_unknown，冻结后的 `FrozenEditSpec` 与完成事件在同一 SQLite 事务提交；
